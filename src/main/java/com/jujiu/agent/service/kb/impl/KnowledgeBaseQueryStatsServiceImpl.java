...
public class KnowledgeBaseQueryStatsServiceImpl implements KnowledgeBaseQueryStatsService {
    ...
    public KbQueryStatsResponse getQueryStats(Long userId, Long kbId, Integer windowDays, ZoneId zoneId, Integer topN) {
        ...
        // 2. 统计问答摘要
        Map<String, Object> summary = kbQueryLogRepository.aggregateSummary(userId, kbId);
        Map<String, Object> feedback = kbQueryFeedbackRepository.aggregateQuality(userId, kbId);
        List<Map<String, Object>> ratingRows = kbQueryFeedbackRepository.aggregateRatingDistribution(userId, kbId);

        // 3. 统计趋势数据
        LocalDateTime now = LocalDateTime.now();
        List<KbTrendPointResponse> trend7Days = toTrend(kbQueryLogRepository.aggregateTrend(userId, kbId, now.minusDays(7)));
        List<KbTrendPointResponse> trend30Days = toTrend(kbQueryLogRepository.aggregateTrend(userId, kbId, now.minusDays(30)));
